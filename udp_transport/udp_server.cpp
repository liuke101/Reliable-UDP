...
void UdpServer::wait_for_ack() 
{
    ...
    // 接收客户端返回的ack数据包
    while ((n = recvfrom(sockfd_, buffer, MAX_PACKET_SIZE, 0, (struct sockaddr *)&client_address, &addr_size)) <= 0) 
    {};
...
void UdpServer::calculate_rtt_and_time(struct timeval start_time, struct timeval end_time) 
{
    ...
    // 计算采样 往返时间 rtt
    long sample_rtt = (end_time.tv_sec * 1000000 + end_time.tv_usec) -  (start_time.tv_sec * 1000000 + start_time.tv_usec);
    // 计算平滑 往返时间
    smoothed_rtt_ = smoothed_rtt_ + 0.125 * (sample_rtt - smoothed_rtt_);
    // 计算平滑的RTT 与 最新 RTT 的差距
